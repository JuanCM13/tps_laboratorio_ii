...
{
    public delegate void ListaLlenaDelegado(String mensaje);

    public class Carrito<T> : IComun
    {
        private List<T> listadoProductos;
        private Int32 limiteCompras;
        public static event ListaLlenaDelegado LimiteAlcanzado_Event;
        ...
        /// ACTUALIZACION: EVENTO Y DELEGADO
        /// Si el carrito esta lleno, ademas de lanzar la excepcion de carrito lleno, lanza un evento por si el programador
        /// desea utilizarlo, el cual lanza un mensaje con el cual el programador, puede mostrar mensajes de error en consola
        /// o windows forms, deshabilitar botones etc, si no se implementa un manejador, no rompe, pero siempre que se llene se lanza
        /// siempre y cuando se le haya cargado un manejador, osea que no sea null
        /// </summary>
        ...
        public bool AgregarProducto(T producto)
        {
            ...
            if(!(producto is null))
            {
                ...
                {
                    if( !(LimiteAlcanzado_Event is null))
                    {
                        LimiteAlcanzado_Event.Invoke("Lista llena! Elimine algun producto o \nrealice la compra para vaciar el carrito!");
                    }                    
